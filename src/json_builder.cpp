...
{
    if (node_stac_.back()->IsString() && (*(node_stac_.end() - 2))->IsDict())
    {
        auto& key = node_stac_.back();
        node_stac_.pop_back();
        const_cast<Dict&>(node_stac_.back()->AsDict()).insert(std::make_pair(std::move(const_cast<std::string&>(key->AsString())), std::move(value)));
    }
    else if (node_stac_.back()->IsArray())
    {
        const_cast<Array&>(node_stac_.back()->AsArray()).push_back(std::move(value));
    }
    else
    {
